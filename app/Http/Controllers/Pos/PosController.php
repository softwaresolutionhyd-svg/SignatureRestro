...
class PosController extends Controller
{
    ...
    private function posIndexViewData(Request $request, PosSession $session): array
    {
        ...
        if ($request->filled('resume_order')) {
            ...
            if ($resumedOrder) {
                ...
                $full = $this->posOrderDetailsPayload($resumedOrder, false);
                $pendingBillsDetail = $pendingBillsDetail->map(
                    fn ($row) => (int) ($row['id'] ?? 0) === (int) $resumedOrder->id ? $full : $row
                );
            }
        }
    ...
    public function sync(Request $request): JsonResponse
    {
        ...
        if ($request->filled('resume_order_id')) {
            ...
            if ($resumedOrder !== null) {
                $full = $this->posOrderDetailsPayload($resumedOrder, false);
                $pending = $pending->map(
                    fn ($row) => (int) ($row['id'] ?? 0) === (int) $full['id'] ? $full : $row
                )->values();
                $resumed = [
                    'id' => $full['id'],
                    'items' => $full['items'] ?? [],
    ...
    private function heldOrdersForSession(PosSession $session, ?User $user = null): \Illuminate\Support\Collection
    {
        ...
        // List/sync only needs card fields — skip line items (opened bills fetch via resume_order_id).
        if ($heldOrders->isNotEmpty()) {
            $heldOrders->load(['table:id,name', 'user:id,name']);
    ...
    public function checkout(PosCheckoutRequest $request): RedirectResponse|JsonResponse
    {
        ...
        try {
            ...
            $sameCartSessionIds = $this->resolvePosBillSessionIds($session, $checkoutUser);
            ...
            if ($request->type === 'sale' && ! $payingSpecificDraft) {
                ...
                if ($twinPaid !== null) {
                    if ($existingDraft && (int) $existingDraft->id !== (int) $twinPaid->id) {
                        // Orphan twin draft — drop it so it cannot be paid again.
                        try {
                            SyncAwareDelete::relation($existingDraft->items());
    ...
    public function hold(PosCheckoutRequest $request): RedirectResponse|JsonResponse
    {
        ...
        try {
            ...
            $sameCartSessionIds = $this->resolvePosBillSessionIds($session, $holdUser);
            ...
            try {
                ...
                Cache::put($cacheKey, (int) $order->id, now()->addMinutes(20));
                Cache::put($itemsCacheKey, (int) $order->id, now()->addMinutes(20));
    ...
    private function findRecentSameCartDraft(
        ...
        // Prefer same user, but managers may re-punch after cashier — still collapse twins.
        if ($userId) {
