    ...
    private function sendBlockingWithRetry(string $ip, int $port, string $payload, int $timeoutSeconds, int $attempts = 2)
    {
        ...
        for ($attempt = 1; $attempt <= $attempts; $attempt++) {
            ...
            while ($written < $len) {
                ...
                if ($chunk === false || ! empty($meta['timed_out'])) {
                    $ok = false;
                    $lastError = new RuntimeException(sprintf('Printer %s:%d timeout.', $ip, $port));
                    break;
                }
                if ($chunk === 0) {
                    usleep(20000);
                    ...
                    if ($chunk === false || $chunk === 0) {
                        $ok = false;
                        $lastError = new RuntimeException(
                            sprintf('Printer %s:%d par data bhejne mein masla.', $ip, $port)
                        );
                        break;
                    }
                }
