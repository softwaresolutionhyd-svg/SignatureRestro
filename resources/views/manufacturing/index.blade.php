...
@section('title', __('Manufacturing') . ' — ' . config('app.name'))

@section('content')
<div class="mb-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
    <div>
        <h4 class="fw-bold mb-0">{{ __('Manufacturing') }}</h4>
        <div class="text-secondary small">{{ __('BoMs define what to consume; production orders move stock via inventory (FIFO).') }}</div>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="card kpi-card shadow-sm h-100">
            <div class="card-body">
                <div class="kpi-label">{{ __('Active BoMs') }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card kpi-card shadow-sm h-100">
            <div class="card-body">
                <div class="kpi-label">{{ __('Total BoMs') }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card kpi-card shadow-sm h-100">
            <div class="card-body">
                <div class="kpi-label">{{ __('Draft orders') }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card kpi-card shadow-sm h-100">
            <div class="card-body">
                <div class="kpi-label">{{ __('Completed (month)') }}</div>
...
<div class="card shadow-sm">
    <div class="card-header bg-white d-flex align-items-center justify-content-between">
        <span class="fw-semibold">{{ __('Recent production orders') }}</span>
        <a href="{{ route('manufacturing.orders.index') }}" class="btn btn-sm btn-outline-primary">{{ __('View all') }}</a>
    </div>
    <div class="table-responsive">
        <table class="table table-hover mb-0 align-middle">
            ...
            @empty
                <tr><td colspan="6" class="text-center text-secondary py-4">{{ __('No orders yet. Create a BoM, then start a production order.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
