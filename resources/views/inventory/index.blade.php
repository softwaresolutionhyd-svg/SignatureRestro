...
@section('title', __('Inventory') . ' - ' . config('app.name'))
@section('page_title', __('Inventory'))
        ...
        <div class="col-12 col-md-6 col-xl-3">
            <div class="card kpi-card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="small text-secondary">{{ __('All products') }}</div>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-xl-3">
            <div class="card kpi-card shadow-sm">
                <div class="card-body">
                    <div class="kpi-label">{{ __('Active Products') }}</div>
                    <div class="kpi-value">{{ fmt_num($kpis['active_products'], 0) }}</div>
                    <div class="small text-secondary">{{ __('Available for stock moves') }}</div>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-xl-3">
            <div class="card kpi-card shadow-sm">
                <div class="card-body">
                    <div class="kpi-label">{{ __('Total On Hand') }}</div>
                    <div class="kpi-value">{{ fmt_num($kpis['on_hand_total'], 3) }}</div>
                    <div class="small text-secondary">{{ __('Sum of quantities') }}</div>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6 col-xl-3">
            <div class="card kpi-card shadow-sm">
                <div class="card-body">
                    <div class="kpi-label">{{ __('Moves Today') }}</div>
                    <div class="kpi-value">{{ fmt_num($kpis['moves_today'], 0) }}</div>
                    <div class="small text-secondary">{{ __('Stock activity') }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                @if($kpis['out_of_stock'] > 0)
                    <span class="text-danger fw-semibold">{{ $kpis['out_of_stock'] }} {{ __('out of stock') }}</span>&nbsp;&nbsp;
                @endif
                @if($kpis['low_stock'] > 0)
                    <span class="text-warning fw-semibold">{{ $kpis['low_stock'] }} {{ __('below reorder level') }}</span>
                @endif
            </div>
        </div>
        <a href="{{ route('inventory.low-stock') }}" class="btn btn-warning btn-sm">{{ __('View Low Stock Report') }} →</a>
    </div>
    @endif

    <div class="card shadow-sm mt-3">
        <div class="card-header bg-white d-flex align-items-center justify-content-between">
            <div class="fw-semibold">{{ __('Recent stock moves') }}</div>
            <a href="{{ route('inventory.moves.index') }}" class="btn btn-sm btn-outline-primary">{{ __('View all') }}</a>
        </div>
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                @empty
                    <tr><td colspan="7" class="text-center text-secondary py-4">{{ __('No moves yet.') }}</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
