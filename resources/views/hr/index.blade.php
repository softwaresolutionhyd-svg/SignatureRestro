...
@section('title', __('HR') . ' — ' . config('app.name'))
...
<div class="row g-3 mb-4">
    @php
        $ribbon = [
            ['key' => 'employees', 'label' => __('Active Employees'), 'color' => '#ec4899'],
            ['key' => 'present_today', 'label' => __('Present Today'), 'color' => '#22c55e'],
            ['key' => 'pending_leave', 'label' => __('Pending Leave'), 'color' => '#f59e0b'],
            ['key' => 'payroll_draft', 'label' => __('Payroll Draft (This Month)'), 'color' => '#6366f1'],
        ];
    @endphp
...
<div class="row g-4">
    <div class="col-lg-7">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white fw-semibold d-flex justify-content-between align-items-center">
                <span>{{ __('Recent Leave Requests') }}</span>
                <a href="{{ route('hr.leave.index') }}" class="small">{{ __('View all') }}</a>
            </div>
            <div class="table-responsive">
                <table class="table table-hover table-sm mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="5" class="text-center text-secondary py-4">{{ __('No leave requests yet.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="col-lg-5">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white fw-semibold d-flex justify-content-between align-items-center">
                <span>{{ __('Recently Added Employees') }}</span>
                <a href="{{ route('employees.index') }}" class="small">{{ __('View all') }}</a>
            </div>
            <div class="list-group list-group-flush">
                ...
                @empty
                <div class="list-group-item text-secondary text-center py-4">{{ __('No employees yet.') }}</div>
                @endforelse
            </div>
        </div>
    </div>
